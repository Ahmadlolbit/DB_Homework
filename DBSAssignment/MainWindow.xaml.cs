...
namespace DBSAssignment
{
    public partial class MainWindow : Window
    {
        ...
        {
            InitializeComponent();
            userTableNames = GetUserTableNames(LoginWindow.connectionString);
        ...
        static List<string> GetUserTableNames(string connectionString)
        {
            ...
            try
            {
                ...
                using (OracleCommand command = new OracleCommand(query, LoginWindow.connection))
                {
        ...
        private void LoadData(string tableName)
        {
            ...
            try
            {
                using (OracleCommand command = new OracleCommand(sqlQuery, LoginWindow.connection))
                {
        ...
        private void Select_btn_Click(object sender, RoutedEventArgs e)
        {
            if (comboTables.SelectedItem != null)
            {
                StringBuilder sqlBuilder = new StringBuilder($"SELECT * FROM {dataTable.TableName} WHERE ");
                ...
                foreach (DataColumn column in dataTable.Columns)
                {
                    if (column.ColumnName != null)
                    {
                        ...
                        if (columnsInformation.Children[counter] is TextBox textBox &&
                            double.TryParse(textBox.Text, out double number))
                        {
                            condition = $"{column.ColumnName} = {number}";
                        }
                        else if (columnsInformation.Children[counter] is DatePicker datePicker &&
                                 datePicker.SelectedDate.HasValue)
                        {
                            condition =
                                $"{column.ColumnName} = TO_DATE('{datePicker.SelectedDate.Value:dd-MMM-yy HH:mm:ss tt}', 'DD-Mon-YY HH:MI:SS AM')";
                        }
                        else if (columnsInformation.Children[counter] is TextBox tb && !string.IsNullOrEmpty(tb.Text))
                        {
                            condition =
                                $"{column.ColumnName} = '{tb.Text.Replace("'", "''")}'"; // Prevent SQL injection
                        }
                ...
                try
                {
                    using (OracleCommand command = new OracleCommand(sql2, LoginWindow.connection))
                    {
                        using (adapter = new OracleDataAdapter(command))
                        {
                            dataTable = new DataTable();
        ...
        private void IUD(string sqlStmt, int op)
        {
            ...
            command.Connection = LoginWindow.connection;
